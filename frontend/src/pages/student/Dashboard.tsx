...
// ✨ FIX: Corrected import paths assuming standard src structure and aliases
import { useAuth } from "@/context/AuthContext"; // Use alias if configured
import api from "@/services/api";             // Use alias if configured
...
// ✨ UPDATED: This interface now matches the data sent by the updated backend API
...
  end_date: string | null;
  // ✨ Added the nested current_approver object
  current_approver: {
    full_name: string;
  } | null;
}
...
  // Maps the API data to the format needed by the UI
...
    } else if (r.start_date) {
      // Handle case where multi-day might only have start date initially
      duration = formatDate(r.start_date);
    }
...
      duration: duration,
      // Use the fetched approver name, provide fallback text if null or approved/rejected
      authority: (r.status === 'APPROVED' || r.status === 'REJECTED')
                   ? 'Completed'
                   : r.current_approver?.full_name || 'Pending Assignment',
    };
  };
...
                        <StatusBadge
                            // Cast the status to the specific types expected by StatusBadge
                          status={request.status as "approved" | "pending" | "under_review" | "rejected" | "pending_hod_exception"}
                          size="md"
                        />
                      </div>
...
                          {/* Now correctly displays the fetched authority name */}
